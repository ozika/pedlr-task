 ...
 jsPsych.plugins['multiple-slider'] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
      ...
      html += ".jspsych-multiple-slider-statement { display:block; font-size: 16px; padding-top: 40px; margin-bottom:10px; }"+
        ...
        ".jspsych-multiple-slider-opt-label { line-height: 1.1em; color: black; }"+
        ".jspsych-multiple-slider-opts:before { content: ''; position:relative; top:11px; /*left:9.5%;*/ display:block; background-color:black; height:4px; width:100%; }"+
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        for(var j=0; j < question.labels.length; j++){
          ...
          html += '<span style="text-align: center; font-size: 80%; color: black">'+question.labels[j]+'</span>';
          html += '</div>'
        }
        html += '</div>';
        ...
        if (trial.show_value) {
          if (i == 0) {
            html += '<input type="range" class="jspsych-slider" value="' + question.slider_start + '" min="' + question.min + '" max="' + question.max + '" step="' + question.step+'" style="width: 100%;" class="jspsych-html-slider-response-response" id="jspsych-html-slider-response-response-'+i+'" name="Q'+i+'" data-name="'+question.name+'" " oninput="this.nextElementSibling.value = this.value"></input><output style="color:black;font-size: 25px;">' + question.slider_start + '</output>';
          } else {
            html += '<input type="range" class="jspsych-slider" value="' + question.slider_start + '" min="' + question.min + '" max="' + question.max + '" step="' + question.step+'" style="width: 100%;" class="jspsych-html-slider-response-response" id="jspsych-html-slider-response-response-'+i+'" name="Q'+i+'" data-name="'+question.name+'" " oninput="this.nextElementSibling.value = this.value">+/- </input><output style="color:black;font-size: 25px;">' + question.slider_start + '</output>';
          }
         // html += '<input type="range" class="jspsych-slider" value="' + question.slider_start + '" min="' + question.min + '" max="' + question.max + '" step="' + question.step+'" style="width: 100%;" class="jspsych-html-slider-response-response" id="jspsych-html-slider-response-response-'+i+'" name="Q'+i+'" data-name="'+question.name+'" " oninput="this.nextElementSibling.value = this.value"></input><output style="color:'+param.c2+';font-size: 30px;">' + question.slider_start + '</output>';
        } else {
          html += '<input type="range" class="jspsych-slider" value="' + question.slider_start + '" min="' + question.min + '" max="' + question.max + '" step="' + question.step + '" id="jspsych-html-slider-response-response-'+i+'" name="Q'+i+'" data-name="'+question.name+'"></input><output></output>';
        }
